...
def run() -> int:
    ...
    if check.returncode:
        raise DemoError('Gateway configuration validation failed. No public listener was started.')
    processes = []
    stopping = False
    ...
    try:
        ...
        gate_env.update(DEMO_GATE_ORIGIN=cfg['origin'], DEMO_GATE_KEY_FILE=str(cfg['gate_key']),
                        DEMO_ACCESS_PASSWORD=os.environ['DEMO_ACCESS_PASSWORD'], DEMO_GATE_PORT=str(GATE_PORT))
        ...
        print('Wavelink fictional client demo started. One application worker; browser administration; no native Admin.', flush=True)
        while not stopping:
...
if __name__ == '__main__':
    try:
        ...
        # No environment dump, request URI, credentials or database content.
        if isinstance(exc, DemoError):
            print('Demo startup stopped: ' + str(exc), file=sys.stderr)
        else:
            print('Demo startup stopped. Review configuration, source compatibility and persistent storage; no replacement project was opened.', file=sys.stderr)
        raise SystemExit(2)

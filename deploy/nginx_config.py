...
def render(
    ...
    *,
    ...
    if (
        bind not in ('0.0.0.0', '127.0.0.1')
        or any(c in str(runtime) for c in ' \t\n;{}"')
    ):
...
http {{
    ...
    # Platform ingress must attest TLS.
    # Ambiguous values fail closed.
    map $http_x_forwarded_proto $demo_tls_ok {{
        default 0;
        https 1;
    }}
    ...
    server {{
        ...
        location = /__demo/check {{
            return 404;
        }}

        location = /__demo/required {{
            return 404;
        }}
...
'''
    ...
    substitutions = '''
            ...
            sub_filter 'ONBOARD HUB'
                       'PRIVATE ONLINE DEMO';

            sub_filter 'Need access or a password reset? Ask the PC hub administrator.'
                       'Need access or a password reset? Ask your online project administrator.';

            sub_filter 'Keep the Windows hub running and awake. Phones must be able to reach its IP address on the approved local network. No internet is needed during local use.'
                       'This demonstration runs on the hosting server. No Windows Admin or laptop needs to stay open. An internet connection is required for this separate online demo.';

            sub_filter 'This address is saved in the PC Admin console.'
                       'This is the configured online demonstration address.';

            sub_filter 'To change it, synchronise work, stop the hub, enter the correct PC IP, Save address and restart.'
                       'The hosting operator manages the online address. Preserve unsent work before changing domains.';
'''
